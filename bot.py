...
Real tools: Jupiter, Helius, Raydium, Smithii
"""
import logging
import asyncio
...
async def router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if d == "m_main":
        ...
        if not mint:
            await query.edit_message_text("â Set token first!", reply_markup=launch_kb())
            return
        ...
        try:
            ...
            await query.edit_message_text(f"â `{str(e)}`", parse_mode="Markdown", reply_markup=sell_kb())
        ...
        if not mint:
            await query.edit_message_text("â No token!", reply_markup=buy_kb())
            return
        ...
        try:
            ...
            await query.edit_message_text(f"â `{str(e)}`", parse_mode="Markdown", reply_markup=buy_kb())
        ...
        if not mint:
            await query.edit_message_text("â Set token first!", reply_markup=volume_kb())
            return
        ...
        if not mint:
            ...
            return
        await query.edit_message_text("â³ Counting...", reply_markup=None)
        ...
        if not mint:
            ...
            return
        await query.edit_message_text("â³ Searching...", reply_markup=None)
...
def run_bot():
    ...
    if not TELEGRAM_TOKEN:
        print("[BOT ERROR] No TELEGRAM_TOKEN!", flush=True)
        return
    ...
    # CRITICAL FIX: Always use polling on Render free tier.
    # Webhook mode conflicts with Flask on the same port.
    # Polling + UptimeRobot hitting /health is the reliable pattern.
